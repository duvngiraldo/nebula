...
class TestConversionRate:
    ...
    def test_returns_correct_rates(self, clean_leads_df):
        result = conversion_rate(clean_leads_df)
        mar1 = result.filter(pl.col("fecha_creacion") == date(2026, 3, 1))
        mar2 = result.filter(pl.col("fecha_creacion") == date(2026, 3, 2))
        assert mar1["conversion_rate"].item() == 0.5
        assert mar2["conversion_rate"].item() == 1.0

    def test_ventas_count(self, clean_leads_df):
        result = conversion_rate(clean_leads_df)
        assert result["ventas"].to_list() == [1, 1]
...
class TestEffectiveCallsPerDay:
    ...
    def test_returns_correct_dates(self, clean_calls_df):
        result = effective_calls_per_day(clean_calls_df)
        assert result["fecha_llamada"].to_list() == [date(2026, 3, 1), date(2026, 3, 2)]

    def test_returns_correct_counts(self, clean_calls_df):
        result = effective_calls_per_day(clean_calls_df)
        assert result["total_llamadas_efectivas"].to_list() == [2, 1]


class TestEffectiveCallsPerAgent:

    def test_returns_agent_names(self, clean_calls_df, users):
        result = effective_calls_per_agent(clean_calls_df, users)
        names = sorted(result["nombre"].to_list())
        assert names == ["Alice", "Bob", "Charlie"]

    def test_returns_correct_counts(self, clean_calls_df, users):
        result = effective_calls_per_agent(clean_calls_df, users)
        alice = result.filter(pl.col("id_usuario") == 301)["total_llamadas_efectivas"].item()
        bob = result.filter(pl.col("id_usuario") == 302)["total_llamadas_efectivas"].item()
